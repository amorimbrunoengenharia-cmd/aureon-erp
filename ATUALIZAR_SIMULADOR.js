// ========================================
// SCRIPT DE ATUALIZAÇÃO DO SIMULADOR
// ========================================
...
const fornecedoresAtualizados = fornecedores.map(f => {
  if (!f.itens || f.itens.length === 0) {
    const numItens = Math.floor(Math.random() * 5) + 3; // 3 a 7 itens
    const itens = [];
...
if (despesas.length === 0) {
  despesas = [
    ...
    },
    {
      id: Date.now() + 5,
      categoria: 'Marketing',
      descricao: 'Campanhas Google Ads',
      valor: 650,
      data: new Date().toISOString(),
      status: 'pendente'
    },
...
console.log('\n%c🔄 Pressione F5 para recarregar a página', 'font-size: 14px; font-weight: bold; color: #f59e0b; background: #fef3c7; padding: 8px; border-radius: 4px;');
